...
| ~~rpgmaker-graphics-fix.user.js (已废弃)~~ | **RPG Maker MZ Graphics Fix** |	请启用`enable-webgl-draft-extensions`特性，Edge前往`edge://flags/#enable-webgl-draft-extensions`<br>~~修复 RPG Maker MZ 游戏中 Failed to initialize graphics 错误，创建可靠的 Graphics 模拟对象并修补 SceneManager.initGraphics，确保游戏正常启动。~~ | ~~安装 (已废弃)~~ |
